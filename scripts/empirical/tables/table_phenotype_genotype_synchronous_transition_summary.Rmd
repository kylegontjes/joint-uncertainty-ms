...
variable_transitionstats  <- c("num_concordant_transitions")
...
transition_stats_wide <- transition_stats %>% arrange(genotype)%>% 
  pivot_wider(
    names_from = genotype,
    values_from = c(num_concordant_transitions,),
...
transition_stats_wide <- transition_stats_wide %>% select(pheno_cluster,paste0(levels(transition_stats$genotype),"_num_concordant_transitions"))

colnames(transition_stats_wide) <- gsub("_num_concordant_transitions"," concordant transitions",colnames(transition_stats_wide)) %>% gsub("pheno_cluster","Phenotype axis",.)
```
