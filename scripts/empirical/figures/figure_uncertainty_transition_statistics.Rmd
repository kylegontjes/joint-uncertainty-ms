...
best_joint_parent_child_transitions_simple <- best_joint_parent_child_transitions %>% select(transitions,gains,gains_tip,losses,losses_tip,continuations_present,continuations_absent,singletons,singleton_isolates,clusters,cluster_isolates,cluster_size_median,cluster_size_max)
   
colnames(best_joint_parent_child_transitions_simple) <- c("Transitions","Gain Events","Gain Events (Tips)","Loss Events","Loss Events (Tips)","Continuations (Present)","Continuations (Absent)","Singletons","Singleton isolates","Clusters","Cluster isolates","Cluster size (median)","Cluster size (max)")
...
transition_data_simple <- alt_recons %>% select(transitions,gains,gains_tip,losses,losses_tip,continuations_present,continuations_absent,singletons,singleton_isolates,clusters,cluster_isolates,cluster_size_median,cluster_size_max,cluster)
colnames(transition_data_simple) <- c("Transitions","Gain Events","Gain Events (Tips)","Loss Events","Loss Events (Tips)","Continuations (Present)","Continuations (Absent)","Singletons","Singleton isolates","Clusters","Cluster isolates","Cluster size (median)","Cluster size (max)","Cluster")
```

```{r,fig.width=9.75,fig.height=7.8}
...
ggsave("./figures/empirical/phyloAMR_alternative_reconstruction_transition_statistics.png",dpi = 900,bg = 'white',plot = transitional_data,width=9.75,height=7.8)
```
