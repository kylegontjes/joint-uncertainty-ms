...
# Transition data
```{r}
best_joint_parent_child <- readRDS("./data/empirical/phyloAMR/representative_joint_uncertainty_parent_child_dfs.RDS") 
state_transition_statistics <- readRDS("./data/empirical/phyloAMR/joint_states_reconstruction_transition_statistics.RDS")
...
```{r}
# Curate best
best_joint_parent_child_transitions <- phyloAMR::asr_transition_analysis(best_joint_parent_child[['best_joint']])
...
state_transition_statistics$cluster <- dbres$cluster %>% as.numeric()
transition_data_simple <- state_transition_statistics %>% select(-reconstruction,-gain_frequency,-loss_frequency,-continuation_present_frequency,-continuations,,-continuation_absent_frequency,-total_edges)
...
ggsave("./figures/empirical/phyloAMR_alternative_reconstructoin_transitions_statistics.png",dpi = 600,bg = 'white',plot = transitional_data,width=15,height=12.5)
```
