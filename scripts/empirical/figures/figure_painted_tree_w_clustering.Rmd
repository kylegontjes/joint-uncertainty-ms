...
```{r,fig.width=12.5,fig.height=8.75} 
...
painted_trees_w_cluster_calls <- lapply(variables,FUN=function(x){
  ...
  p2 <- gheatmap(p1.1,data=dat  %>% select(paste0(x,"_asr_cluster_renamed")) %>% `colnames<-`("Clustering"),width = 0.1,colnames_position = 'top',colnames_angle=90, colnames_offset_y = 0.25, hjust = 0, color = NA,offset=0.000025)  + cluster_scale_v + ylim(NA,500)
  p2
})
...
tracing_states_on_tree_wo_legend <- cowplot::plot_grid(plotlist = painted_trees_w_cluster_calls_wo_legends,nrow=2,labels =c("Best joint reconstruction",paste0("Reconstruction axis ",1:n_clusters,"    ")),label_x = -0.25,label_size = 14)
...
ggsave("./figures/empirical/ancestral_states_on_phylogeny.png",dpi = 900,bg = 'white',plot = tracing_states_on_tree,width=12.5,height=8.75)
```