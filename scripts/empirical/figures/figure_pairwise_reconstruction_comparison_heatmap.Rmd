...
matrix_of_vals <- pairwise_comparisons_ordered %>%
    ...
    summarise(value = num_concordant_transitions) %>%  # or median(), sum(), etc
...
matrix_of_vals <- pairwise_comparisons_ordered %>%
    group_by(genotype_reconstruction_axes, phenotype_reconstruction) %>%
    summarise(value = num_discordant_transitions) %>%  # or median(), sum(), etc
    pivot_wider(names_from = genotype_reconstruction_axes, values_from = value) 
...
```{r}
png("./figures/empirical/pairwise_reconstruction_synchronous_transition_comparisons_concordant_heatmap.png", width=12, height=8, units="in", res=900)
draw(
  A,
...
png("./figures/empirical/pairwise_reconstruction_synchronous_transition_comparisons_disconcordant_heatmap.png", width=12, height=8, units="in", res=900)
draw(
  B,
  heatmap_legend_list = list(merged_legends),
  annotation_legend_list = NULL
