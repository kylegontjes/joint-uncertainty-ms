...
#Packages
packages <- c("tidyverse",'corHMM','proxy','dbscan','Rtsne','phyloAMR','kableExtra','ComplexHeatmap','RColorBrewer') 
...
```{r}
pairwise_comparisons <- readRDS("./data/empirical/phyloAMR/pairwise_synchronous_transitions_comparisons.RDS")  %>% subset(genotype_reconstruction != 'focal_states') %>% subset(genotype %in% c("AA552","OmpK36_c25t","OmpK36_L3_mutations","OmpK36_promoter_IS","OmpK36_disruptive","PBP_any","RamR","RamA"))
```

## Recode genotypes
```{r}
pairwise_comparisons$genotype <- factor(
pairwise_comparisons$genotype,levels = rev(c("AA552","OmpK36_c25t","OmpK36_L3_mutations","OmpK36_promoter_IS","OmpK36_disruptive","PBP_any","RamR","RamA")))
...
# Synchronous
```{r,fig.width=12, fig.height=8,results='hide',echo=FALSE}
...
phen_colors <- setNames(
  c("black", brewer.pal(length(unique(dbcluster_names)), "Set1")),
  unique(factor(matrix_of_vals$cluster))
)
