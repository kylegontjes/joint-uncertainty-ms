...
```{r}
focal <- readRDS("./data/empirical/joint_reconstruction/focal_asr.RDS")
...
nodelabels(pch = 21,  cex = 1, bg = c("white", "grey80")[focal$states], col = "grey70")
nodelabels(pch = 21,  cex = 1, bg = c("white", "black")[focal$states], col = "black", node = c(417, nodes_to_col))
...
pdf("./figures/empirical/empirical_joint_uncertainty_reconstructions.pdf", height = 10, width = 5)
par(mar=c(0,0,0,0))
m <- t(state_df_matrix)[, order(cluster_assignments)]
m <- m[, ncol(m):1]
colors <- ifelse(m == 2, "white", "black")
...
corHMM:::plot_stacked_densities(joint_ci$lnliks, clusters, cols = c(clust_cols), main = "")
...
for(i in 1:optimal_k_gap){
  ...
  nodelabels(pch = 21, bg = c("white", "black")[as.numeric(phy_tmp$node.label)], cex = 1)
}
dev.off()
```
