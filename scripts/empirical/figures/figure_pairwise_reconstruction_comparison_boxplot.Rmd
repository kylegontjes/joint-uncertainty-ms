...
pairwise_comps_sync_all_geno$genotype,levels = rev(c("AA552","OmpK36_c25t","OmpK36_L3_mutations","OmpK36_promoter_IS","OmpK36_disruptive","PBP_any","RamR","RamA")))
...
## B. Proportion of phenotype gain events with genotype gain events 
...
B <- ggplot(pairwise_comparisons,
       aes(x = synchronous_gains_prop,
           ...
           fill = as.factor(pheno_cluster))) +
geom_boxplot(position = position_dodge(width = 0.8)) +
labs(x = "Proportion of phenotype gain events with genotype gain",
     y = "",
     fill = "Phenotype axes") +
scale_fill_brewer(palette = "Set1",guide = guide_legend(nrow=1)) +
theme(axis.text.x = element_text(angle = 45, hjust = 1)) + theme_bw()  + scale_x_continuous(breaks = c(0,0.05,0.1,0.15,0.20,0.25)) + xlim(NA,0.251) + theme(legend.position = 'bottom')
...
C<- ggplot(pheno_gain_explanation_df,
       aes(x = prop_pheno_gains_w_geno_gain,
           ...
           fill = as.factor(pheno_cluster))) +
geom_boxplot(position = position_dodge(width = 0.8)) +
labs(x = "Proportion of phenotype gain events with >=1 genotype gain",
     y = "",
     fill = "Phenotype axes") +
...
```{r,fig.width=9,fig.height=7.5} 
D <- ggplot(pairwise_comps_sync_all_geno,
       aes(x = synchronous_gains_prop,
           y = as.factor(genotype),
           fill = as.factor(pheno_cluster))) +
geom_boxplot(position = position_dodge(width = 0.8)) +
labs(x = "Proportion of genotype gain events with phenotype gain",
     y = "",
     fill = "Phenotype axes") +
scale_fill_brewer(palette = "Set1",guide = guide_legend(nrow=1)) +
...
legend_fig <- gontjesr::get_legend(A + theme(legend.positoin = 'bottom'))

data_fig <- cowplot::plot_grid(A + theme(legend.position = 'none'),B + theme(legend.position = 'none'),C + theme(legend.position = 'none'),D + theme(legend.position = 'none'),ncol=2)
figure <- cowplot::plot_grid(data_fig,legend_fig,ncol=1,rel_heights = c(1,0.1))
