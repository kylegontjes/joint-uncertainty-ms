...
labs(x = "No. concordant synchronous transitions",
     y = "Genotype",
     fill = "Phenotype axes") +
...
B <- ggplot(pairwise_comparisons,
       aes(x = num_discordant_transitions,
           ...
           fill = as.factor(pheno_cluster))) +
geom_boxplot(position = position_dodge(width = 0.8)) +
labs(x = "No. discordant synchronous transitions",
     y = "Genotype",
     fill = "Pheno Cluster") +
scale_fill_brewer(palette = "Set1") +
theme(axis.text.x = element_text(angle = 45, hjust = 1)) + theme_bw()
...
AB <- cowplot::plot_grid(A + theme(legend.position = 'none'),B + theme(legend.position = 'none'),ncol=1,labels="AUTO")
legend_fig <- cowplot::get_legend(A)  %>% cowplot::plot_grid()
  
AB_w_legend <- cowplot::plot_grid(AB,legend_fig,ncol=2,rel_widths = c(1,0.2))

AB_w_legend

ggsave("./figures/empirical/pairwise_reconstruction_synchronous_transition_comparisons_boxplot.png",dpi = 900,bg = 'white',plot = AB,width=10,height=10)
```
